...
#include <stddef.h>
#include <stdbool.h>
#include "kvm.h" /** @brief 需要訪問 VM 狀態以標記根對象 */

/** @brief KObjType and KObjHeader are defined in kvm.h to avoid circular dependency */

/** @brief GC 狀態 */
typedef struct {
    KObjHeader* head;         /** @brief 所有分配對象的鏈表頭 */
    size_t bytes_allocated;   /** @brief 當前已分配的總字節數 */
    size_t next_gc_threshold; /** @brief 觸發下一次 GC 的閾值 */
    KVM* vm;                  /** @brief 反向引用 VM 以獲取根 (Roots) */
    
    /** @brief 統計信息 */
    size_t gc_count;          /** @brief GC 觸發次數 */
} KGC;
